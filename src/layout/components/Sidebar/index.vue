...
<template>
  <div>
    ...
    <!-- menu -->
    <Menu ref="menuElement" :theme="sideTheme" class="i-layout-menu-side i-scrollbar-hide" :class="{
      'i-layout-menu-side-collapse': isCollapsed,
      'i-layout-menu-side-has-logo': hasLogo,
    }" width="auto" :accordion="menuAccordion" :active-name="activeMenu" :open-names="openNames">
      <!-- 菜单已展开 -->
      <!-- eslint-disable -->
      <template v-if="!isCollapsed">
        <side-menu-item v-for="(route, index) in sidebarRoutes" :key="route.path + index" :item="route"
          :base-path="route.path" />
      </template>

      <!-- 菜单已折叠 -->
      <template v-else>
        <side-menu-collapse v-for="(route, index) in sidebarRoutes" :key="route.path + index" :item="route" top-level
          :base-path="route.path" />
      </template>
    </Menu>
  </div>
...
const initOpenNames = () => {
  ...
  matched.forEach(element => {
    openNameArray.push(element.path);
  });
